...
export default function TheMenu({ userId }: TheMenuProps) {
  return (
    <>
      <ToggleMenu className="absolute right-3 -top-2 peer z-30 sm:hidden" />
      ...
      <div className="mt-[250px] mb-[500px] hidden peer-has-[:checked]:flex sm:peer-has-[:checked]:hidden flex-col gap-12 justify-start items-center fixed inset-0 z-20 screen-w screen-h">
        <Link className="outline-none hover:underline uppercase font-semibold tracking-wide" href={paths.createNotodoPage(userId)} prefetch>create</Link>
        <Link className="outline-none hover:underline uppercase font-semibold tracking-wide" href={paths.rewardListPage(userId)} prefetch>consume</Link>
      </div>
    </>
  )
}
