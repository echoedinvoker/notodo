  ...
  return (
    <>
      ...
      <div className="mt-[250px] mb-[500px] hidden peer-has-[:checked]:flex sm:peer-has-[:checked]:hidden flex-col gap-12 justify-start items-center fixed inset-0 z-20 screen-w screen-h">
        <Link className="hover:underline uppercase font-semibold tracking-wide" href={paths.createNotodoPage(userId)}>create</Link>
        <Link className="hover:underline uppercase font-semibold tracking-wide" href={paths.rewardListPage(userId)}>consume</Link>
      </div>
    </>
  )
}
